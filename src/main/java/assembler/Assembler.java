...
public class Assembler {
    ...
    public static void main(String[] args) throws IOException, TokenizingException, MismatchingVersionException, MappingException, LinkingException {
        ...
        for (LinkedToken token : tokens) {
            if (token instanceof LinkedMemoryBlock memoryBlock) {
                ...
                if (operation.getInstruction().getSize() == 2) {
                    binary[operation.getAddress() + 1] = operation.getArg() != null ? operation.getArg().byteValue() : 0;
                }
            }
        }
