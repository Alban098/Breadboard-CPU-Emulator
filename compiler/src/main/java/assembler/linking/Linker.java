...
/**
 * This component will execute the Linking step of the compilation. It will take all mapped token
...
public class Linker {
  ...
  /**
   * Execute the linking step of the compilation
   *
   * @param mappingResult
        ...
        if (arg.startsWith("$(")) {
          ...
          if (operation.getToken().getInstruction().acceptLabels()) {
            ...
            throw new LinkingException(
                "ERROR:? - '"
                    + arg
                    + "' Labels can only be used with branching instruction, not on '"
                    + operation.getToken().getInstruction().getFormat()
                    + "'");
          }
        } else if (arg.startsWith("*")) {
  ...
  private Integer handleDereferenceAlias(
      List<MappedVariable> variables, List<MappedConstant> constants, String arg)
      throws LinkingException {
  ...
  private static Integer handlePlainVariable(List<MappedVariable> variables, String arg)
      throws LinkingException {
  ...
  private static Integer handlePlainConstant(List<MappedConstant> constants, String arg)
      throws LinkingException {
